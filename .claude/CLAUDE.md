...
Current step: Phase 5 (SSO/workforce) implemented on 2026-07-12 (whole-phase mode, per user), awaiting user review. `/gcp-check` done: `--login-config` flag overrides the `auth/login_config_file` property (doc-quoted), login-config JSON fields confirmed, federated console URL `https://auth.cloud.google/signin/.../providers/...?continueUrl=https://console.cloud.google/` (federated domain has no .com; project deep-link in continueUrl is best-effort, undocumented). Built: `core/workforce.rs` (audience parsing), `Context::identity()` (principal:// prefix detection), `hop login --sso/--login-config`, federated `hop console`, identity line in `hop status`, workforce-aware re-auth (login config threaded through auth_flow). 90 tests green; e2e simulated with fixture login-config + principal account (real-IdP round-trip still deferred: user must supply a workforce test tenant, PLAN Phase 5 verification). Phase 4 note (user-tested OK): console/impersonate live. Next: Phase 6 (Homebrew release via /release-prep) on user go.
