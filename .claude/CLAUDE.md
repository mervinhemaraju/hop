...
The 7-phase milestone plan (skeleton, local context read/write, auth + projects, console + impersonation, SSO / workforce identity, Homebrew release, post-release improvements backlog) is complete through Phase 6, with Phase 7 in progress. Key decisions already made:
...
Current step: Phase 5 (SSO/workforce) COMPLETE, user-confirmed 2026-07-13 (real-IdP round-trip done by user against their workforce tenant with a login-config file + custom browser script). Built in Phase 5: `core/workforce.rs` (audience parsing), `Context::identity()` (principal:// prefix detection), `hop login --sso/--login-config`, federated `hop console` (`auth.cloud.google` sign-in URL), identity line in `hop status`, workforce-aware re-auth (login config threaded through auth_flow). Side feature (2026-07-13, user-requested, user-tested OK): `browser` setting in settings.json (custom browser command for login flows and `hop console`; BROWSER env var overrides; `~/` expanded; `GcloudCli::new(Option<PathBuf>)` sets BROWSER on the gcloud child, `CustomBrowser` adapter spawns the command with the URL as sole arg, `core::settings::effective_browser` holds the precedence). 98 tests green. README fully restructured 2026-07-13 (intro/features, install, Getting started covering gcloud configuration creation + SSO login-config setup, usage per command, settings paths, troubleshooting). Phase 6 IN PROGRESS (2026-07-13): tap decision = single `mervinhemaraju/homebrew-tap` repo holding all formulas (install: `brew install mervinhemaraju/tap/hop`); added LICENSE (MIT), `.github/workflows/ci.yml` (fmt/clippy/test on macOS+Linux, compile-only Windows), `.github/workflows/release.yml` (on `v*` tag: version-check vs Cargo.toml, builds aarch64/x86_64 darwin + x86_64 musl tar.gz + windows zip, SHA256SUMS, drafts the GitHub release for the user to publish; third-party actions SHA-pinned, checkout persist-credentials off per SAST). /release-prep 0.1.0 ran 2026-07-13: preflight green (clean tree, 98 tests, cargo audit clean on 144 crates, cargo-audit 0.22.2 installed locally), version stays 0.1.0, CHANGELOG.md created, formula draft at packaging/homebrew/hop.rb (sha256 placeholders to fill from the release's SHA256SUMS asset). Artifacts come from release.yml on tag push (per tooling decision), not local builds. Phase 6 COMPLETE, user-verified 2026-07-13: v0.1.0 published on GitHub (release workflow ran on tag; user filled checksums), public `mervinhemaraju/homebrew-tap` repo live with `Formula/hop.rb`, tap README, and an `update-formula` workflow (manual workflow_dispatch with version input; fetches the release's SHA256SUMS and rewrites the formula via `.github/scripts/update_formula.py`; tested idempotent against the live v0.1.0 release + bump/failure paths). `brew install mervinhemaraju/tap/hop` works on the user's machine. `.gitattributes` excludes `.claude/`+`.github/` from source archives. Per-release runbook: publish GitHub release, then run update-formula in the tap. Phase 7 IN PROGRESS (2026-07-15, toward 0.2.0 on branch `feature/console-pickers-and-reauth-fix`): `hop console` gained interactive configuration + project pickers (read-only; never activates a configuration, builds a Context from the chosen one; shared project-listing helper `commands/project_source.rs` extracted from switch and reused by both); picker prompts parameterized ("Open configuration:"/"Open project:" vs switch's "Switch to..."); `--show-principal` flag on switch + console (the config picker hides each entry's account/principal by default, showing `name  project  (active)`, and reveals it on the flag); fixed workforce (SSO) re-auth that passed the `principal://` account positionally to `gcloud auth login` while a login config was present, making gcloud refresh the expired token (`invalid_grant`) instead of a fresh federated sign-in: `GcloudCli::login` now omits the account positional whenever a login config is set, extracted into a testable `login_args`. 113 tests green. PLAN.md removed (milestone plan complete through Phase 6; this file is the living status).
